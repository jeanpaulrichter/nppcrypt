...
bool CPreferences::load(const TCHAR* path, CurrentOptions& current)
{
	if(!path)
		false;
	...
	try {
		...
		if (strcmp(NPPC_PREFFILE_HEADER_1010, theader) == 0)
		{
			...
			unicode::utf8_to_wchar(t_string.c_str(), t_string.size(), files.extension);
...
bool CPreferences::save(CurrentOptions& current)
{
	...
	try {
		...
		unicode::wchar_to_utf8(files.extension.c_str(), files.extension.size(), t_string);
		ts = t_string.size();
...
{
	if(!lstrlen(key.label) || keys.size() >= NPPC_HMAC_MAX_KEYS)
		return false;
	keys.push_back(key);
	return true;
}
